...
def send_affiliation_expired_to_parish(self, church_affiliation_id: uuid.UUID,) -> None:
    """
    Notifica a Sede (from_church) de que o convite foi expirado.

    Envia para o e-mail da Sede:
      - Nome da igreja que expirou  
    ...
    try:
        ...
        expired_church_name = (affiliation.to_church.full_name if affiliation.to_church else affiliation.invited_church_full_name) or ""

        context = {
            "parish_name": affiliation.from_church.full_name or "",
            "expired_church_name": expired_church_name,
        ...
        EmailService.send_html_email(
            subject=f"Convite expirado — {expired_church_name} não faz mais parte da sua rede",
            to_email=affiliation.to_church.user.email if affiliation.to_church else affiliation.invited_email,
            template_name="community/emails/affiliation_expired_to_parish.html",
            context=context,
        )

        logger.info(
            "Affiliation expired notification sent to parish. "
            "request_id=%s parish=%s expired_by=%s",
            affiliation.id,
        ...
        logger.exception("Error sending affiliation expired notification. request_id=%s", church_affiliation_id,)
