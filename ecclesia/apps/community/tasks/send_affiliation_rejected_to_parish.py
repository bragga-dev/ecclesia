...
def send_affiliation_rejected_to_parish(
    self,
    church_affiliation_id: uuid.UUID,
) -> None:
    """
    Notifica a Sede (from_church) de que o convite foi rejeitado.

    Envia para o e-mail da Sede:
      - Nome da igreja que rejeitou
    ...
    try:
        ...
        # Nome da igreja que rejeitou — pode vir de to_church (online)
        # ou de invited_church_full_name (offline, antes de ter to_church)
        rejected_church_name = (
        ...
        context = {
            ...
            "rejected_church_name": rejected_church_name,
        ...
        EmailService.send_html_email(
            subject=f"Convite rejeitado — {rejected_church_name} agora faz parte da sua rede",
            to_email=affiliation.from_church.user.email,
            template_name="community/emails/affiliation_rejected_to_parish.html",
            context=context,
        )

        logger.info(
            "Affiliation rejected notification sent to parish. "
            "request_id=%s parish=%s rejected_by=%s",
            affiliation.id,
        ...
        logger.exception(
            "Error sending affiliation rejected notification. request_id=%s",
