"""
Testes para a task send_confirme_affiliation_online_invite.
"""
import uuid
...
class TestSendConfirmeAffiliationOnlineInvite:
    """Testes para a task de confirmação de convite online de afiliação."""

    def setup_method(self):
        """Configuração inicial para cada teste."""
        self.church_affiliation_id = uuid.uuid4()
        self.from_church_name = "Igreja Teste"
        self.to_church_email = "igreja_destino@exemplo.com"
        self.invited_church_full_name = "Igreja Destino"

    @patch('ecclesia.apps.community.tasks.send_confirme_affiliation_online_invite.get_church_affiliation_request_by_id')
    @patch('ecclesia.apps.community.tasks.send_confirme_affiliation_online_invite.EmailService')
    @patch('ecclesia.apps.community.tasks.send_confirme_affiliation_online_invite.logger')
    def test_send_confirme_affiliation_online_invite_success(
        self, mock_logger, mock_email_service, mock_get_affiliation
    ):
        """Testa envio de confirmação de convite online com sucesso."""
        # Mock da afiliação
        mock_affiliation = self._create_mock_affiliation()
        mock_get_affiliation.return_value = mock_affiliation

        # Executar task
        send_confirme_affiliation_online_invite(self.church_affiliation_id)
        ...
        assert call_args["subject"] == "Solicitação de Afiliação"
        assert call_args["to_email"] == self.to_church_email
        assert call_args["template_name"] == "community/emails/send_confirme_affiliation_online_invite.html"
        assert call_args["context"]["from_church_name"] == self.from_church_name
        assert call_args["context"]["invitation_url"] == (
            f"{settings.FRONTEND_URL}/dashboard/community/affiliations/{mock_affiliation.id}"
        )
    ...
    @patch('ecclesia.apps.community.tasks.send_confirme_affiliation_online_invite.get_church_affiliation_request_by_id')
    @patch('ecclesia.apps.community.tasks.send_confirme_affiliation_online_invite.logger')
    def test_send_confirme_affiliation_online_invite_not_found(
    ...
    @patch('ecclesia.apps.community.tasks.send_confirme_affiliation_online_invite.EmailService')
    @patch('ecclesia.apps.community.tasks.send_confirme_affiliation_online_invite.logger')
    def test_send_confirme_affiliation_online_invite_retry_on_failure(
        self, mock_logger, mock_email_service, mock_get_affiliation
    ):
        ...
        with patch.object(send_confirme_affiliation_online_invite, 'retry', side_effect=Retry()):
    ...
    def _create_mock_affiliation(self):
        """Cria um mock de afiliação."""
        ...
        # Mock da igreja destino
        mock_affiliation.to_church = MagicMock()
        mock_affiliation.to_church.user = MagicMock()
        mock_affiliation.to_church.user.email = self.to_church_email
