"""
Testes para a task send_affiliation_request.
"""
import uuid
...
class TestSendAffiliationRequest:
    """Testes para a task de solicitação de afiliação."""
    ...
    @patch('ecclesia.apps.community.tasks.send_affiliation_request.get_church_affiliation_request_by_id')
    @patch('ecclesia.apps.community.tasks.send_affiliation_request.EmailService')
    @patch('ecclesia.apps.community.tasks.send_affiliation_request.logger')
    def test_send_affiliation_request_success(
        self, mock_logger, mock_email_service, mock_get_affiliation
    ):
        """Testa envio de solicitação de afiliação com sucesso."""
        # Mock da afiliação
        mock_affiliation = self._create_mock_affiliation()
        mock_get_affiliation.return_value = mock_affiliation

        # Executar task
        send_affiliation_request(self.church_affiliation_id)
        ...
        assert call_args["template_name"] == "community/emails/send_affiliation_request.html"
    ...
    @patch('ecclesia.apps.community.tasks.send_affiliation_request.get_church_affiliation_request_by_id')
    @patch('ecclesia.apps.community.tasks.send_affiliation_request.logger')
    def test_send_affiliation_request_not_found(
    ...
    @patch('ecclesia.apps.community.tasks.send_affiliation_request.EmailService')
    @patch('ecclesia.apps.community.tasks.send_affiliation_request.logger')
    def test_send_affiliation_request_retry_on_failure(
        self, mock_logger, mock_email_service, mock_get_affiliation
    ):
        ...
        with patch.object(send_affiliation_request, 'retry', side_effect=Retry()):
