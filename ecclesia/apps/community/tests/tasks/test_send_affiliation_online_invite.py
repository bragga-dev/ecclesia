"""
Testes para a task send_affiliation_online_invite.
"""
import uuid
...
class TestSendAffiliationOnlineInvite:
    """Testes para a task de convite online de afiliação."""
    ...
    @patch('ecclesia.apps.community.tasks.send_affiliation_online_invite.get_church_affiliation_request_by_id')
    @patch('ecclesia.apps.community.tasks.send_affiliation_online_invite.EmailService')
    @patch('ecclesia.apps.community.tasks.send_affiliation_online_invite.logger')
    def test_send_affiliation_online_invite_success(
        self, mock_logger, mock_email_service, mock_get_affiliation
    ):
        """Testa envio de convite online com sucesso."""
        # Mock da afiliação
        mock_affiliation = self._create_mock_affiliation_online()
        mock_get_affiliation.return_value = mock_affiliation

        # Executar task
        send_affiliation_online_invite(self.church_affiliation_id)
        ...
        assert call_args["template_name"] == "community/emails/affiliation_online_invite.html"
    ...
    @patch('ecclesia.apps.community.tasks.send_affiliation_online_invite.get_church_affiliation_request_by_id')
    @patch('ecclesia.apps.community.tasks.send_affiliation_online_invite.logger')
    def test_send_affiliation_online_invite_not_found(
    ...
    @patch('ecclesia.apps.community.tasks.send_affiliation_online_invite.EmailService')
    @patch('ecclesia.apps.community.tasks.send_affiliation_online_invite.logger')
    def test_send_affiliation_online_invite_retry_on_failure(
        self, mock_logger, mock_email_service, mock_get_affiliation
    ):
        ...
        with patch.object(send_affiliation_online_invite, 'retry', side_effect=Retry()):
    ...
    def _create_mock_affiliation_online(self):
        """Cria um mock de afiliação online."""
